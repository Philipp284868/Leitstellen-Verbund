...
import { z } from "zod";
import { inBounds } from "../shared/germany/projection";
...
import { alarmSchema } from "../simulation/schema";
import { approach } from "../shared/travel";
import {
  Auth,
  passwordSchema,
  profileNameSchema,
...
export function startServer(
  ...
  let activated = !process.env.LV_READY_TOKEN;
  const worldGeneration = String(
    db.sql
  ...
  async function handleHttp(req: IncomingMessage, res: ServerResponse) {
    res.setHeader("X-World-Generation", worldGeneration);
    ...
    try {
      ...
      if (!activated)
        return reply(res, 503, { error: "Neue Version wird noch geprüft." });
      ...
      if (path.startsWith("/api/")) {
        ...
        if (path === "/api/operator/status" && req.method === "GET") {
          if (
            !db.sql
              .prepare("SELECT 1 FROM game_operators WHERE user_id=?")
              .get(session.user_id)
          )
            return reply(res, 403, {
              error: "Spieladministrator erforderlich.",
            });
          return reply(res, 200, {
            accounts: Number(
              db.sql.prepare("SELECT count(*) n FROM users").get()!.n,
            ),
            worldGeneration,
            simulationHealthy: !failed,
          });
        }
  ...
  io.use((socket, next) => {
    if (!activated) return next(Error("Neue Version wird noch geprüft."));
    if (
      socket.handshake.auth.generation &&
      socket.handshake.auth.generation !== worldGeneration
    )
      return next(Error("Neue Spielwelt. Bitte erneut anmelden."));
...
if (process.argv[1] && /(?:^|[\\/])index\.js$/.test(process.argv[1])) {
  ...
  if (process.connected)
    process.send?.({
      ...
      host: c.host,
      port: c.port,
      token: process.env.LV_READY_TOKEN,
      commit: process.env.LV_BUILD_COMMIT,
      instance: process.env.LV_INSTANCE_ID,
      generation: app.worldGeneration,
    });
  let closing = false;
  const stop = () => {
    if (closing) return;
  ...
  process.on("message", (message) => {
    if (
      ...
      "token" in message &&
      message.token === process.env.LV_READY_TOKEN
    ) {
      app.activate();
      process.send?.({ type: "activated", token: process.env.LV_READY_TOKEN });
    }
    if (
      message &&
      typeof message === "object" &&
