...
import { useWorldGeneration } from "./storage";
import {
  RouteSnapshotDecoder,
  type RouteSnapshotFrame,
} from "../shared/germany/snapshot";
import { createId } from "./ids";
import type { GameMode } from "../shared/mode";
...
export async function api(
  ...
  const generation = r.headers.get("X-World-Generation");
  if (generation) await useWorldGeneration(generation);
  if (!r.ok) {
