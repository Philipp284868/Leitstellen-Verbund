...
const generatedBackup = new RegExp(
  `^game-[0-9]{10,17}-${uuid}${sqliteSuffix}$`,
  "i",
);
const migrationCopy = new RegExp(
  `^pre-migration-(?:v2-)?[0-9]{10,17}(?:-${uuid})?${sqliteSuffix}$`,
  "i",
);
...
function regular(path) {
  ...
  if (!info.isFile() || info.isSymbolicLink() || info.nlink !== 1)
    throw Error(
      `Unsicherer Dateityp/Verknüpfung; nichts an dieser Datei löschen: ${path}`,
    );
  return info;
}
function readSmall(path) {
  ...
  try {
    if (!sameFile(info, fstatSync(fd)))
      throw Error("Datei während der Prüfung geändert.");
...
function parseRecord(raw, label) {
  try {
    ...
    throw Error(`${label} ist beschädigt. Keine automatische Freigabe.`);
  }
}
...
function lockData(dataDir) {
  ...
  if (previous) {
    ...
    if (!dead)
      throw Error(
        "Prozess existiert noch oder Berechtigung unklar. Reset abgebrochen; Spiel zuerst stoppen.",
      );
    const current = readSmall(path);
    if (
      !current ||
      current.text !== previous.text ||
      !sameFile(current.info, previous.info)
    )
  ...
  const assertOwned = () => {
    ...
    if (
      ...
      !sameFile(current.info, owned.info)
    )
...
export function hardReset({
  ...
  if (!confirmed)
    throw Error(
      "Bestätigung --confirm-delete-all-player-data fehlt. Keine Daten gelöscht.",
    );
  if (!/^[a-z0-9][a-z0-9_-]{3,63}$/.test(requestId || ""))
    throw Error(
      "--request benötigt eine eindeutige Kennung aus 4–64 Kleinbuchstaben/Ziffern/_/-.",
    );
  const app = realpathSync(programDir),
    requestedData = resolve(dataDir);
  directory(app);
  ...
  if (!outside(app, data) || !outside(data, app) || dirname(data) === data)
    throw Error(
      "Daten- und Programmordner müssen getrennt sein; kein Wurzel-/übergeordneter Ordner.",
    );
  if (
    JSON.parse(readFileSync(resolve(app, "package.json"), "utf8")).name !==
    "leitstellen-verbund"
  )
    throw Error("Kein Leitstellen-Verbund-Programmordner.");
  const appIdentity = metadata(app),
    dataIdentity = metadata(data);
  ...
  if (existing) {
    ...
    if (
      receipt.format !== "leitstellen-hard-reset-v1" ||
      receipt.request !== requestId
    )
      throw Error("Reset-Beleg ungültig. Keine Daten gelöscht.");
    if (receipt.state === "completed")
      return { repeated: true, deletedFiles: 0, dataDir: data };
    throw Error(
      "Dieser Reset wurde unterbrochen. Nichts erneut gelöscht. App gestoppt lassen; Zustand und Lock prüfen.",
    );
  }
  ...
  try {
    ...
    for (const name of readdirSync(data))
      if (
        ...
        migrationCopy.test(name) ||
        restoreCopy.test(name)
      )
        paths.push(resolve(data, name));
    ...
    const adminTemporary = new RegExp(
      `^admin-konto\\.json\\.${uuid}\\.tmp$`,
      "i",
    );
    for (const name of readdirSync(app))
      if (OBSOLETE_FILES.includes(name) || adminTemporary.test(name))
        paths.push(resolve(app, name));
    ...
    for (const { path, info } of plan) {
      lock.assertOwned();
      const nowApp = metadata(app),
        nowData = metadata(data);
      if (
        nowApp?.ino !== appIdentity.ino ||
        nowApp?.dev !== appIdentity.dev ||
        nowData?.ino !== dataIdentity.ino ||
        nowData?.dev !== dataIdentity.dev ||
        nowApp.isSymbolicLink() ||
        nowData.isSymbolicLink()
      )
        throw Error("Ordner während des Resets geändert.");
      if (dirname(path) === backups) {
        const current = metadata(backups);
        if (
          !current?.isDirectory() ||
          current.isSymbolicLink() ||
          current.ino !== backupsInfo.ino ||
          current.dev !== backupsInfo.dev
        )
          throw Error("Sicherungsordner während des Resets geändert.");
      }
      if (!sameFile(regular(path), info))
        throw Error("Datei während des Resets geändert.");
      unlinkSync(path);
      syncDirectory(dirname(path));
      receipt.deletedFiles++;
    }
    if (backupsInfo && readdirSync(backups).length === 0) {
      rmdirSync(backups);
      syncDirectory(data);
    }
    ...
    return {
      repeated: false,
      deletedFiles: receipt.deletedFiles,
      dataDir: data,
    };
  } catch (error) {
    if (pending)
      throw Error(
        `Reset nicht vollständig bestätigt. Keine Erfolgsmeldung: ${error.message} Wartungssperre bleibt; nicht einfach neu starten.`,
      );
    throw error;
  } finally {
...
if (invoked) {
  try {
    ...
    if (
      args.length !== 3 ||
      args[0] !== "--confirm-delete-all-player-data" ||
      args[1] !== "--request"
    )
      throw Error(
