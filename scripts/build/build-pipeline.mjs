...
export async function buildApplication({ incremental = false } = {}) {
  ...
  const commonInputs = [
    ...
    "pnpm-workspace.yaml",
    "tsconfig.json",
    "scripts/build/build-pipeline.mjs",
    "scripts/build/build-cache.mjs",
    "scripts/build/source-graph.mjs",
    "CHANGELOG.md",
    "src/client/changelog.ts",
    "scripts/build/build-changelog.mjs",
    ...[
      ".env",
      ".env.local",
      ".env.production",
  ...
  if (!failure) {
    await run(["scripts/build/build-changelog.mjs"]);
    await run(["scripts/build/check-bundles.mjs"]);
    let commit = null,
      dirty = true;
    try {
