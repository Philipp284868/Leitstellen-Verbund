...
Setup: Node.js 24, `node scripts/amp-setup.mjs`. `npm run dev` startet Vite-Hot-Reload und einen separaten lokalen Backendprozess. Die lokale Entwicklung verwendet ../leitstellen-verbund-development-data, niemals die Produktionsdaten. Der Client aktualisiert automatisch; serverseitige Änderungen mit `node scripts/build-server.mjs` neu bauen.
