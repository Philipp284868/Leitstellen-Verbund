...
`tests/e2e/resource-actions.spec.ts` durchläuft zusätzlich zwei tatsächliche Browserfälle: Versetzung an eine eigene FF-Wache ohne Personalwanderung, einmaliger Fahrzeugverkauf mit vollständigem Erhalt der übrigen Personen, anschließender leerer Standortverkauf und Serverneustart. Der zweite Fall bestätigt Ausbaukosten und Bauwartezeit, zusätzliche Stellplätze sowie eine tatsächliche technische Erweiterung mit anschließendem Rüstwagenkauf; die passende qualifizierte Besatzung ist sofort vorhanden. Beide Fälle bestanden in Edge (2/2, 36,644 Sekunden); es wurden keine Produktregeln für die Prüfung geändert. Details: `.tools/test-runs/2.21-resource-actions.json`.
