...
- Der Python-DEM-Satz enthält acht erfolgreiche Tests, einschließlich identischer Quellen-Lock-Prüfsummen bei frischen Git-Checkouts mit drei verschiedenen Zeilenendeneinstellungen. Der XML-/OSM-Indeximport wurde zusätzlich mit einer kleinen bekannten Topologie geprüft.
...
Die GPU-losen Linux-CI-Rechner verwenden für Firefox einen echten virtuellen Bildschirm (Xvfb) und Mesa-Softwarerendering. Im ersten Lauf blockierte der Firefox-Headless-Modus WebGL2 bereits vor der Karteninitialisierung; Trace und Screenshot enthielten den entsprechenden Grafikfehler. `LV_CI_SOFTWARE_GL=1` erlaubt Software-WebGL ausschließlich im isolierten Playwright-Testprofil. Es ersetzt keine Kartenfunktion, überspringt keinen Test und verändert keine Browsereinstellungen der Spieler. Der [von Playwright beschriebene Xvfb-Betrieb](https://playwright.dev/docs/ci#running-headed) bildet einen Browser mit Bildschirm ab; die Messwerte bleiben ausdrücklich CI-Softwarerendering und keine Hardware-GPU-Benchmarks.
