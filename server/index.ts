...
const registerSchema = loginSchema
  .extend({
    ...
    station: z.string().trim().min(1).max(48),
...
export function startServer(
  ...
  const online = () =>
    new Set(
      Array.from(io.sockets.sockets.values()).map((s) => String(s.data.user)),
    );
  ...
  function ip(req: IncomingMessage) {
    ...
    // Trust only an explicitly configured immediate peer; the proxy must overwrite X-Real-IP.
  ...
  const http = createServer(async (req, res) => {
    ...
    try {
      ...
      if (path.startsWith("/api/")) {
        ...
        if (path === "/api/login" || path === "/api/register") {
          ...
          const raw = await body(req),
            parsed = loginSchema.passthrough().parse(raw);
          ...
          if (path === "/api/register") {
            ...
            const row = db.sql
              ...
              .get(data.username);
            // Equal-cost derivation for unknown users as well.
            const encoded = row
              ? String(row.password)
        ...
        if (path === "/api/me" && req.method === "GET")
          return reply(res, 200, {
            ...
            csrf: session.csrf,
            ...game.view(session.user_id, online()),
          });
        ...
        if (path === "/api/action" && req.method === "POST") {
          if (failed)
            return reply(res, 503, {
              error:
                "Simulation pausiert wegen Speicherfehler. Administrator informieren.",
            });
        ...
        if (path === "/api/export" && req.method === "GET")
          return reply(res, 200, {
            ...
            version: 1,
            exportedAt: Date.now(),
            save: game.view(session.user_id, online()).save,
          });
        if (
          ...
          return reply(res, 200, { invite: auth.invite(session.user_id) });
        if (
          ...
          db.audit(session.user_id, "database-backup");
          return reply(res, 200, { file: file.split(/[\\/]/).at(-1) });
        }
      ...
      // Never send raw SQL, passwords, cookies, or internal filenames to clients.
      reply(
        res,
  ...
  const timer = setInterval(() => {
    ...
    try {
      ...
      io.emit(
        "notice",
        "Server-Simulation pausiert. Administrator informieren.",
      );
    }
  }, 1000);
...
if (process.argv[1] && /(?:^|[\\/])index\.js$/.test(process.argv[1])) {
  ...
  console.log(
    `Leitstellen-Verbund bereit: ${c.host}:${c.port} · ${c.publicUrl}`,
  );
  let closing = false;
  const stop = () => {
    if (closing) return;
