...
import { capacity, hospital, readiness, tick } from "../src/shared/engine";
...
it("Sichtung benötigt medizinische Kräfte und priorisiert geeignete Transporte samt Klinikverteilung", () => {
  ...
  expect(hospital(s, m.pos, 1, m, s.vehicles[5])!.id).toBe("public:way:100000");
  expect(() =>
    majorCommand(
      s,
...
it("angenommene Nachbarkräfte bleiben in Bereitstellung bis zur autorisierten Abschnittszuweisung", async () => {
  ...
  try {
    ...
    advanceUntil(
      w.game,
      () => w.db.all().get(w.peer)!.vehicles[0].status === "scene",
    );
