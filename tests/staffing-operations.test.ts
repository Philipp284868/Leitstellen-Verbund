...
import { capacity, readiness, tick } from "../src/shared/engine";
...
import { approach, tripLabel } from "../src/shared/travel";
import { vehiclePosition } from "../src/shared/vehicle-position";
import { distance, nearest, route } from "../src/shared/world";
...
it("umfährt gesperrte Kanten oder wartet tatsächlich; Verkehrsmeldung teleportiert kein Fahrzeug", () => {
  ...
  s.environment!.roads = [
    {
      id: "jam-test",
      roadId: v.journey!.motion!.find((p) => p.meters > 0)!.edge,
      kind: "jam",
      edge,
      start: s.time,
      until: s.time + 300,
