...
async function server() {
  ...
  const a = await app.auth.create(
    ...
    invitation = app.auth.invite(a);
  const b = await app.auth.create("ben", password, "Ben", "Süd", invitation);
...
describe("Autoritativer Server", () => {
  it("liefert Client und API auf einem Port, ohne private Caches oder externe Ressourcen", async () => {
    ...
    expect((await fetch(origin + "/game.sqlite")).status).toBe(404);
  });
  ...
  it("verlangt Einladung, bindet sie einmalig und begrenzt Anmeldung auch nach Neustart", async () => {
    ...
    const invite = app.auth.invite("test");
    expect((await request("register", { ...data, invite })).status).toBe(200);
    expect(
      (await request("register", { ...data, username: "another", invite }))
        .status,
    ).toBe(400);
    for (let i = 0; i < 10; i++)
  ...
  it("verwirft fremde Origins, fehlenden CSRF-Token und frei vorgegebene Kontostände", async () => {
    ...
    const { cookie, csrf } = await login();
    const action = { id: crypto.randomUUID(), action: { type: "relief" } };
  ...
  it("behält Besitz, Aktionsbelege und laufende Zeit über Datenbankneustart und konsistente Sicherung", async () => {
    ...
    expect(db.all().get(id)!.money).toBe(251500);
    expect(db.sql.prepare("PRAGMA user_version").get()!.user_version).toBe(1);
    db.close();
  });
