...
it("normale Spieler ändern nur ihr eigenes Passwort mit aktuellem Passwort und CSRF; alte Sitzungen verfallen", async () => {
  ...
  await app.listen();
  const post = (path: string, data: unknown, cookie = "", csrf = "") =>
  ...
  try {
    ...
    expect(
      (await post("password", { current: password, password: next }, cookie))
        .status,
    ).toBe(403);
    expect(
      (
        await post(
          "password",
          { current: "Incorrect-old-password!", password: next },
          cookie,
          me.csrf,
        )
      ).status,
    ...
    expect(
      (
        await post(
          "password",
          { current: password, password: next },
          cookie,
          me.csrf,
        )
      ).status,
    ...
    expect(
      (await post("login", { username: "spieler", password })).status,
    ).toBe(401);
    const login = await post("login", { username: "spieler", password: next });
    ...
    expect(latest.user.id).toBe(me.user.id);
    expect(latest.user.role).toBe("player");
    expect(latest.save.money).toBe(me.save.money);
  } finally {
    await app.close();
  }
});
